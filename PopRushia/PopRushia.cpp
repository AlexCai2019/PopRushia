...
BEGIN_MESSAGE_MAP(CPopRushiaApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CPopRushiaApp::OnAppAbout)
	// 依據文件命令的標準檔案
...
CPopRushiaApp::CPopRushiaApp() noexcept
{
	...
	SetAppID(_T("PopRushia.AppID.NoVersion"));

	// TODO:  在此加入建構程式碼，
	// 將所有重要的初始設定加入 InitInstance 中
}

// 唯一一個 CPopRushiaApp 物件

CPopRushiaApp theApp;


// CPopRushiaApp 初始化

BOOL CPopRushiaApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CPopRushiaDoc),
		RUNTIME_CLASS(CMainFrame),       // 主 SDI 框架視窗
		RUNTIME_CLASS(CPopRushiaView));
	if (!pDocTemplate)
		return FALSE;
...
int CPopRushiaApp::ExitInstance()
{
	// TODO: 處理其他您已經加入的資源
	AfxOleTerm(FALSE);
